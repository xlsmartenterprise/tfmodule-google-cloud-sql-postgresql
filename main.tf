locals {
  ...
  additional_users_policies = {
    for username, user in local.users : username => {
      allowed_failed_attempts      = try(user.password_policy.allowed_failed_attempts, var.password_policy_config.allowed_failed_attempts)
      enable_failed_attempts_check = try(user.password_policy.enable_failed_attempts_check, var.password_policy_config.enable_failed_attempts_check)
      enable_password_verification = try(user.password_policy.enable_password_verification, var.password_policy_config.enable_password_verification)
      password_expiration_duration = try(user.password_policy.password_expiration_duration, var.password_policy_config.password_expiration_duration)
    }
  }
...
resource "google_sql_user" "default" {
  ...
  password_policy {
    allowed_failed_attempts      = local.default_user_policy.allowed_failed_attempts
    enable_failed_attempts_check = local.default_user_policy.enable_failed_attempts_check
    enable_password_verification = local.default_user_policy.enable_password_verification
    password_expiration_duration = local.default_user_policy.password_expiration_duration
  }
}
...
resource "google_sql_user" "additional_users" {
  ...
  password_policy {
    allowed_failed_attempts      = local.additional_users_policies[each.key].allowed_failed_attempts
    enable_failed_attempts_check = local.additional_users_policies[each.key].enable_failed_attempts_check
    enable_password_verification = local.additional_users_policies[each.key].enable_password_verification
    password_expiration_duration = local.additional_users_policies[each.key].password_expiration_duration
  }
}
