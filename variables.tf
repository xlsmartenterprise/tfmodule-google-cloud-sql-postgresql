...
variable "additional_users" {
  ...
  validation {
    ...
    error_message = "Password is a required field for built_in Postgres users and you cannot set both password and random_password, choose one of them."
  }
}
...
variable "password_policy_config" {
  description = "Default password policy for SQL users"
  type = object({
...
variable "default_user_password_policy" {
  description = "Password policy specifically for the default user. If not set, uses password_policy_config"
  type = object({
    allowed_failed_attempts      = optional(number)
    enable_failed_attempts_check = optional(bool)
    enable_password_verification = optional(bool)
