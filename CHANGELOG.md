...
### Fixed
- Added lifecycle `ignore_changes` for `settings[0].final_backup_config`, `settings[0].version`, `maintenance_version`, and `available_maintenance_versions` on primary instance and read replicas to prevent perpetual Cloud SQL maintenance and backup configuration drift
